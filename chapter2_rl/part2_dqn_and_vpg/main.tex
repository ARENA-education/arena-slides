...
\begin{frame}
	...
	to a supervised learning problem?
		...
		\begin{itemize}
			\pause
			\item Collect many trajectories, dump into a buffer and shuffle until ``i.i.d enough''
		\end{itemize}
		\pause
		\item Rewards are sparse
		\begin{itemize}
			\pause
			\item Q-Learning bootstraps using estimates of the future
			\item VPG already handles exploration as we will see
			\item Can use a critic to learn estimates of the future (tomorrow)
		\end{itemize}
		\pause
...
\begin{frame}{Deep Q-Networks (DQN)}
	\begin{itemize}
		\item The Optimal Q-Value estimate $\widehat{Q}_*(s,a;\theta)$ 
		is now stored as a network, with parameters $\theta$.
		Recall the TD-error for Q-Learning
		$$
		\delta_t = r_{t+1} + \gamma \max_{a'} \widehat{Q}_*(s_{t+1}, a'; \theta) 
		- \widehat{Q}_*(s_t, a_t; \theta)
		$$
		\pause
		\item \textbf{Idea:} Accumulate experience 
		$(s^i,a^i,r^i,s_{\text{new}}^i)$ via interaction, optimise
		$\theta$ to minimise loss $L(\theta) = \mathbb{E}[\sum_t \delta_t^2]$ 
		\pause
		\begin{itemize}
			\item In practice, experience is accumulated in a buffer, and batches
			are sampled at random to make data ``more i.i.d''
			\pause
			\item Also use separate set of parameters $\theta_{\text{target}}$ 
			for the target network, copy weights every so often for stability
		\end{itemize}
		\pause
		\begin{align*}
		L(\theta) &=  \mathbb{E} \left[
		\left( r_{t+1} + \gamma \max_{a'} \widehat{Q}_*(s_{t+1}, a'; \theta_{\text{target}} ) -  \widehat{Q}_*(s_t, a_t; \theta)
		\right)^2 \right] \\
        & = \frac{1}{N}\sum_{i=1}^{N} 
		\left(
		r^i + \gamma \max_{a'} \widehat{Q}_*(s^i_\text{new}, a'; \theta_{\text{target}} ) -  \widehat{Q}_*(s^i, a^i; \theta)
		\right)^2
		\end{align*}
		\pause
...
\begin{frame}
    \centering
    \includegraphics[width=\textwidth]{img/dqn_block.png}
\end{frame}
...
\begin{frame}{Deep Q-Networks (DQN) for Episodic Environments}
	\begin{itemize}
		...
		\begin{align*}
			\delta_t &= y_t - \widehat{Q}_*(s_t, a_t;\theta) \\
		y_t &= \begin{cases}
			r_{t+1} & d_{t+1} = True \\
			r_{t+1} + \gamma \max_{a'} \widehat{Q}_*(s_{t+1}, a'; \theta_{\text{target}}) & d_{t+1} = False 
		\end{cases} \\
		&= r_{t+1} + \gamma \max_{a'} \widehat{Q}_*(s_{t+1}, a'; \theta_{\text{target}}) \llbracket \text{not } d_{t+1} \rrbracket
		\end{align*}
		Loss function is now
		$$
		L(\theta) = \frac{1}{N} \sum_{i=1}^N \left(y_t - \widehat{Q}_*(s_t,a_t; \theta )\right)^2
		$$
		\end{itemize}
\end{frame}
...
%    \end{itemize}
    ...
    distribution $\pi_\bth : \S \to \Delta \A$ over actions, mapping state to a logit vector over actions.
