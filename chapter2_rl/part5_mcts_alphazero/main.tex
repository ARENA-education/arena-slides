...
\begin{frame}
	...
	% \begin{block}{What is the value target $z$ for non-terminal $s$?}
	% 	Each game is played out to the very end, and states are labelled
	% 	with that game's terminal value. The critic is then just a classifer
	% 	that, given a state, tries to predict the terminal value if 
